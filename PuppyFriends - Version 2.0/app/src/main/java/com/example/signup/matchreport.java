...
public class matchreport extends AppCompatActivity {
    ...
    private DatabaseReference databaseReference = firebaseDatabase.getReference();
     ...
     EditText  petsittername1, matchdate1, matchtime1, eat1, review1, bowel1, walk1;
     String matching_id;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        matchdate1 = (EditText)findViewById(R.id.matchdate1);
        eat1 = (EditText)findViewById(R.id.eat1);
        review = (EditText)findViewById(R.id.review1);
        bowel = (EditText)findViewById(R.id.bowel1);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String matchdate = matchdate1.getText().toString();
                ...
                if(petsittername.isEmpty()||matchtime.isEmpty()||eat.isEmpty()||bowel.isEmpty())
                    ...
                    databaseReference.child("matchreport").child(matching_id).child("펫시터이름").setValue(petsittername);
                    databaseReference.child("matchreport").child(matching_id).child("밥먹은시간").setValue(eat);
                    databaseReference.child("matchreport").child(matching_id).child("돌봄시간").setValue(matchtime);
                    databaseReference.child("matchreport").child(matching_id).child("방문날짜").setValue(matchdate);
                    databaseReference.child("matchreport").child(matching_id).child("배변횟수").setValue(bowel);
                    databaseReference.child("matchreport").child(matching_id).child("산책한시간").setValue(walk);
                    databaseReference.child("matchreport").child(matching_id).child("특이사항").setValue(review);
